...
const { JOIN_ROOM, REQ_VIDEO, END_VIDEO, SET_VID, RESP_MSG, SEND_MSG, SET_MSGS, UPD_MSGS, GET_VTIME, SET_VTIME, NAV_VTIME, GET_USERS, SET_PLAY } = require('../src/Constants');
...
io.on('connection', (socket) => {
    ...
    socket.on(JOIN_ROOM, async (data) => {
        ...
        console.log(`"${socket.roomId}", "${socket.userId}", "${socket.realName}", "${socket.dispName}", "${socket.isOwner}"`);
    ...
    socket.on(SEND_MSG, (data) => {
        if (io.sockets.adapter.rooms[socket.roomId]) {
            io.sockets.adapter.rooms[socket.roomId].currMsgList = data.currMsgList;
        }
    ...
    socket.on('disconnect', () => {
        ...
        axios.patch(process.env.REACT_APP_API_URL + "/api/rooms/leave", {roomCode: socket.roomId, guestId: socket.userId})
            ...
            .catch(err => {
                // console.log("Failed (Leaving): " + err);
            });
    });
});
...
mongoose.connect(process.env.ATLAS_URI || 'mongodb://localhost/clique_app', {
